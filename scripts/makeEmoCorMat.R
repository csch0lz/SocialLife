...
###ANTI-ALCOHOL
emo_anti=emo_df_all %>%mutate(val_cond=case_when(grepl('non',condition)~'non-alcoholic',
  ...
  rename(embarrassed="embarrassed (for someone else)",`drinking urge`='drink_rating')

emo_pvals_anti = emo_anti %>% pivot_longer(cols=c(any_of(emo_vars),'embarrassed'),names_to='emotion') %>%
  group_by(emotion) %>% summarise(p=cor.test(value,`drinking urge`)$p.value) %>%
...
emo_cormat_anti=emo_anti %>%
  cor %>%
  get_lower_tri %>%
  melt(.,na.rm=T) %>% 
  filter(Var1=="drinking urge" | Var2=="drinking urge") %>%
  mutate(Var1=factor(Var1,levels=c('scared','shocked','sad','embarrassed','negative','amused','jealous','happy','positive','drinking urge')),
         Var2=factor(Var2,levels=c('scared','shocked','sad','embarrassed','negative','amused','jealous','happy','positive','drinking urge'))
  ) %>%
  left_join(emo_pvals_anti,by=c('Var2'='emotion')) %>%
  mutate(psig=ifelse(Var2=='drinking urge','p<.00025',psig)) 

emo_plot_anti=emo_cormat_anti %>% 
...
final=ggarrange(emo_plot_anti,emo_plot_pro,nrow=1)
